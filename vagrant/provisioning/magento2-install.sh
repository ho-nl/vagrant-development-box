...
[ -f ${HOME_DIR}/${MAGENTO_DIR}/app/etc/env.php ] || $AS_USER bin/magento setup:install \
    --db-host=localhost \
    --db-name=magento2 \
    --db-user=app \
    ...
    --base-url=http://${VAGRANT_HOSTNAME}/ \
    --language=en_US \
...
$AS_USER bin/magento deploy:mode:set developer
$AS_USER rm -rf generated/code generated/metadata
